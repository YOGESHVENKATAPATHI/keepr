...
class _FileManagerScreenState extends State<FileManagerScreen> {
  ...
  List<dynamic> items = [];
  bool loading = false;
  double _clusterUsedBytes = 0;
  double _clusterTotalBytes = 0;
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      try {
        ...
        if (token != null) {
          final stats = await widget.api.getClusterStorageStats(token);
          setState(() {
            _clusterUsedBytes = (stats['usedBytes'] as num).toDouble();
            _clusterTotalBytes = (stats['allocatedBytes'] as num).toDouble();
          });
        }
      } catch (e) {
        debugPrint("Failed to fetch cluster stats: $e");
      }
    } catch (e) {
        ...
        PopupMenuItem(
          ...
          child: Row(children: [
            const Icon(Icons.description_outlined,
                color: Color(0xFF34D399), size: 18),
            const SizedBox(width: 10),
            Text('Notes Studio', style: GoogleFonts.inter(color: Colors.white)),
          ]),
        ),
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('CLUSTER STORAGE', style: GoogleFonts.inter(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                        Text('${(_clusterUsedBytes / (1024 * 1024 * 1024)).toStringAsFixed(2)} GB / ${(_clusterTotalBytes / (1024 * 1024 * 1024)).toStringAsFixed(2)} GB', style: GoogleFonts.inter(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(height: 10),
                    LinearProgressIndicator(
                      value: (_clusterUsedBytes / _clusterTotalBytes).clamp(0.0, 1.0),
